...
public class CiudadJpaController implements Serializable {
    ...
    public void create(Ciudad ciudad) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            em.persist(ciudad); //INSERT INTO public.ciudad (id, nombre) VALUES (4, 'BARRANQUILLA')           
    ...
    public void edit(Ciudad ciudad) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ciudad = em.merge(ciudad); //UPDATE public.ciudad SET nombre = 'BOGOTA D.C.' WHERE id = 1
            em.getTransaction().commit();
        } catch (Exception ex) {
    ...
    public void destroy(Integer id) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Ciudad ciudad = em.getReference(Ciudad.class, id);
            //ciudad.setId(id);
            //em.flush();
            //em.detach(ciudad);
            em.remove(ciudad);//DELETE FROM public.ciudad WHERE id=3
            //em.clear();
            em.getTransaction().commit();
        } finally {
            if (em != null) {
...
//
    ...
    public Ciudad findCiudad(Integer id) {
        EntityManager em = getEntityManager();
        try {
            return em.find(Ciudad.class, id); //SELECT * FROM public.ciudad WHERE id=2
        } finally {
            em.close();
        }
    }
